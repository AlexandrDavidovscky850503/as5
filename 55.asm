...
readFile MACRO file, buf  
    mov bx, file ;идентификатор файла
    mov cx, readLEN ;число байт для чтения
    mov dx, offset buf  ;адрес буфера для приёма файлов
...
resetFile MACRO file  
    xor cx, cx
    xor dx, dx   ; расстояние, на которое надо переместить
    mov bx, file ; идентификатор файла
    mov al, 00h  ;перемещение относительно начала
...
getSize MACRO file  
    xor cx, cx
    xor dx, dx 
    mov bx, file   ; расстояние, на которое надо переместить                                  
    mov al, 02h    ; идентификатор файла                                                      
    mov ah, 42h    ; перемещение относительно конца файла                                           
    int 21h        ; переместить указатель чтения/записи                                       
ENDM                                                                                          
...
openFileR proc 
    ...
    int 21h 
    jc openFail  ;проверка нв открытие файда 
    ret    
endp  
